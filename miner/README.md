...
ComputeHorde Miner.  See repositories main README for ean explanation of what that means.
...
```sh
# 1st tab
$ python -m venv compute_horde_miner
$ source compute_horde_miner
$ ./setup-dev.sh
```
...
mkdir -p ~/repos
cd ~/repos
git init --bare --initial-branch=master miner.git

mkdir -p ~/domains/miner
```

```sh
# locally
git remote add production root@<server>:~/repos/miner.git
...
```sh
# crontab -e
30 0 * * * cd ~/domains/miner && ./bin/backup-db.sh > ~/backup.log 2>&1
```
