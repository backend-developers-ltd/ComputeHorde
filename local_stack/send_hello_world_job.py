...
def create_mt_client_with_tempfiles(
    ...
    cert_pem = client_cert.public_bytes(encoding=serialization.Encoding.PEM).decode(
        "utf-8"
    )
...
async def main() -> None:
    ...
    streaming_job = await compute_horde_client.create_job(
        compute_horde_streaming_job_spec
    )
    await streaming_job.wait_for_streaming(timeout=60)
    logger.info(
        f"Streaming server: {streaming_job.streaming_server_address}:{streaming_job.streaming_server_port}"
    )
    ...
    for attempt in range(1, max_terminate_retries + 1):
        try:
            response = client.get(
                f"https://{streaming_job.streaming_server_address}:{streaming_job.streaming_server_port}/terminate"
            )
            if 200 <= response.status_code < 300:
                logger.info(
                    f"Successfully terminated streaming server: {response.text}"
                )
                break
            else:
                logger.warning(
                    f"Attempt {attempt}: Non-2xx response when terminating streaming server: {response.status_code} - {response.text}"
                )
        except Exception as e:
            logger.warning(
                f"Attempt {attempt}: Exception during streaming server termination: {e}"
            )
        if attempt == max_terminate_retries:
            raise RuntimeError(
                f"Failed to terminate streaming server after {max_terminate_retries} attempts"
            )
        backoff = 2 ** (attempt - 1)
    ...
    if verify_http_output_volumes:
        ...
        if len(job.result.upload_results) != 2:
            raise RuntimeError(
                f"Expected 2 keys in upload results, found {len(job.result.upload_results)}"
            )
