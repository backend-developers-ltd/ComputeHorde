...
tmux send-keys -t horde:Miner "../local_stack/tabs/1.miner.sh" C-m

tmux new-window -t horde -n "ValidatorConnect"
tmux send-keys -t horde:ValidatorConnect "cd $PROJECT_ROOT/validator" C-m
tmux send-keys -t horde:ValidatorConnect "../local_stack/tabs/2.validator-connect.sh" C-m

tmux new-window -t horde -n "ValidatorCelery"
tmux send-keys -t horde:ValidatorCelery "cd $PROJECT_ROOT/validator" C-m
tmux send-keys -t horde:ValidatorCelery "../local_stack/tabs/3.validator-celery.sh" C-m
