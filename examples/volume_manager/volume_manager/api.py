...
async def prepare_volume(request: PrepareVolumeRequest) -> PrepareVolumeResponse:
    ...
    try:
        ...
        if request.volume.volume_type == "multi_volume":
            ...
            logger.info(
                f"Processing multi-volume with {len(request.volume.volumes)} sub-volumes"
            )
            for i, sub_volume in enumerate(request.volume.volumes):
                ...
                mount_path = (
                    ("/volume/" + sub_volume.relative_path)
                    if sub_volume.relative_path
                    else "/volume"
                )
            ...
            logger.info("Processing individual volume")
            volume = create_volume_from_spec(request.volume)
            volume_path = await storage.prepare_volume(volume)

            # Mount to the default volume path
            mount_path = (
                ("/volume/" + request.volume.relative_path)
                if request.volume.relative_path
                else "/volume"
            )
...
async def job_finished(request: JobFinishedRequest) -> JobFinishedResponse:
    ...
    try:
        ...
        logger.exception(
            f"Failed to process job finished notification for {request.job_uuid}"
        )
