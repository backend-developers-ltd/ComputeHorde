...
async def query_miner_split_distributions(miners: list[Miner]) -> dict[str, dict[str, float]]:
    ...
    for miner in miners:
        task = asyncio.create_task(
            _query_single_miner_split_distribution(miner),
            name=f"query_split_{miner.hotkey}"
        )
        tasks.append(task)
...
async def _query_single_miner_split_distribution(miner: Miner) -> dict[str, float]:
        ...
        async with asyncio.timeout(10):  # 10 second timeout
            while True:
                ...
                if response:
                    ...
                    msg = TypeAdapter(MinerToValidatorMessage).validate_json(response)
    ...
    except (TransportConnectionError, asyncio.TimeoutError, Exception) as e:
