...
async def _set_compute_time_allowances(metagraph: MetagraphSnapshot, cycle: Cycle) -> bool:
    ...
    # Get manifests from periodic polling data instead of requesting them from miners
...
async def _get_latest_manifests(miners: list[Miner]) -> dict[str, dict[ExecutorClass, int]]:
    ...
    for miner in miners:
        latest_manifests = await MinerManifest.objects.filter(
            miner=miner,
            online_executor_count__gt=0
        ).order_by('executor_class', '-created_at').alist()
...
async def _poll_miner_manifests() -> None:
    ...
    """
    # Get the latest metagraph to find miners connected to this validator
    try:
        ...
        if manifest:
            for executor_class, executor_count in manifest.items():
                manifest_records.append(
                    MinerManifest(
                        miner=miner,
                        batch=None,  # Periodic manifest (no batch)
            ...
            # Miner didn't respond - store offline status
            # Get last known executor counts for this miner
            last_manifests = await MinerManifest.objects.filter(
                miner=miner,
                batch__isnull=True  # Only periodic manifests
            ).order_by('-created_at').alist()
            
            if last_manifests:
                # Use last known executor counts but mark as offline
                for last_manifest in last_manifests:
                    manifest_records.append(
                        MinerManifest(
                            ...
                            online_executor_count=0,  # Offline
                        )
                    )
