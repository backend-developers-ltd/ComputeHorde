...
def _run_block_scan_with_lock(
    *,
    ...
    scan_func: Callable[[int, SuperTensor, float, float], None],
    task_name: str,
    keep_running: bool,
    ...
    Args:
        ...
        scan_func: Function to call with (current_block, supertensor, start_time, max_run_time) to perform the actual scanning
    ...
    with transaction.atomic(using=settings.DEFAULT_DB_ALIAS):
        try:
            with Lock(lock_type, LOCK_WAIT_TIMEOUT, settings.DEFAULT_DB_ALIAS):
                ...
                with cm as st:
                    scan_func(current_block, st, start_time, max_run_time)
...
def scan_archive_blocks_and_calculate_allowance(
    ...
    _run_block_scan_with_lock(
        ...
        backfilling_supertensor=backfilling_supertensor,
        scan_func=_scan_archive_blocks,
