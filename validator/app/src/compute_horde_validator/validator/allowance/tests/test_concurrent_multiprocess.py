...
# Why tests prefer 'fork' over 'spawn':
# 1. Visibility: fork inherits process memory making already-committed DB rows visible
#     without extra coordination; spawn needs explicit commits & sync to avoid flakiness.
# 2. Flakiness reduction: eliminates timing windows where parent misses child-created booking.
# 3. Focus: isolates validation of transactional row locking, not process bootstrap semantics.
# 4. Portability: falls back to spawn transparently; can add barrier sync later if needed.
# 5. Safety: we close all Django connections pre-fork so children open fresh, independent ones.
...
def test_concurrent_reservations_multiprocess(mp_sync):
    ...
    validator = default_module._allowance_instance.my_ss58_address
