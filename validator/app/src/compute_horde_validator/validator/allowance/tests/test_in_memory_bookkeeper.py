...
def test_empty_allowances(triplets):
    ...
    for triplet in triplets:
        ...
        assert bookkeeper._check_for_spent_blocks(triplet, {1, 2}) == set()
        assert bookkeeper._check_for_invalidated_blocks(triplet, {1, 2}, 1) == set()
        assert bookkeeper._check_for_invalidated_blocks(triplet, {1, 2}, 2) == set()
        assert bookkeeper._check_for_invalidated_blocks(triplet, {1, 2}, 5) == set()
...
def test_register_transaction_and_check_for_spent_blocks(triplets, triplet_idx, expected):
    ...
    assert bookkeeper._check_for_spent_blocks(triplets[triplet_idx], {1, 2, 3}) == expected
...
def test_check_for_invalidated_blocks(triplets, at_block, triplet_idx, expected):
    ...
    result = bookkeeper._check_for_invalidated_blocks(triplets[triplet_idx], {1, 2, 3}, at_block)
