...
with open("/volume/payload.txt", "rb") as file:
    ...
    for i in range(int(data["n"])):
        ...
        cmd = f'hashcat --runtime 99600 --potfile-disable --restore-disable --attack-mode 3 --workload-profile 3 --optimized-kernel-enable --hash-type {algorithm} --hex-salt -1 "?l?d?u" --outfile-format 2 --quiet _payload.txt "{mask}"'
