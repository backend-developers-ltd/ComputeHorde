...
class MinerClient(AbstractMinerClient):
    ...
    async def handle_message(self, msg: BaseRequest):
        ...
        elif isinstance(msg, V0DeclineJobRequest | V0ExecutorFailedRequest | V0ExecutorReadyRequest):
...
def initiate_jobs(netuid, network) -> list[SyntheticJob]:
    ...
    batch = SyntheticJobBatch.objects.create(accepting_results_until=now() + datetime.timedelta(seconds=JOB_LENGTH))
    ...
    return list(
        SyntheticJob.objects.bulk_create(
            [
                SyntheticJob(
                    batch=batch,
                    miner=miner,
                ...
                for miner in miners
                if neurons_by_key[miner.hotkey].axon_info.is_serving
            ]
        )
    )
...
async def _execute_job(
    job: JobBase,
) -> tuple[float | None, V0DeclineJobRequest | V0ExecutorFailedRequest | V0JobFailedRequest | V0JobFinishedRequest]:
...
async def execute_jobs(synthetic_jobs: Iterable[SyntheticJob]):
    tasks = [
        asyncio.create_task(asyncio.wait_for(execute_job(synthetic_job.id), JOB_LENGTH))
