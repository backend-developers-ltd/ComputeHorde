...
2. This function will internally:
    ...
    2. Apply a collateral threshold filter if the dynamic config value `DYNAMIC_MINIMUM_COLLATERAL_AMOUNT_WEI` is > 0. Only miners whose recorded collateral is at least that many Wei are kept. If all miners are filtered out a `NotEnoughAllowanceException` is raised (a warning is logged indicating how many miners were filtered).
