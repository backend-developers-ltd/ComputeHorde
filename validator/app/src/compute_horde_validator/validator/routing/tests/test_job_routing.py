...
@pytest.mark.asyncio
async def test_pick_miner_for_job__picks_a_miner(add_allowance):
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_pick_miner_for_job__all_executors_busy(add_allowance):
    ...
    assert not True  # FIXME: this test should fail
