...
async def test_pick_miner_for_job__skips_busy_miner_based_on_receipts(add_allowance):
    ...
    # Clean any leftover receipts
    await sync_to_async(JobStartedReceipt.objects.all().delete)()
    ...
    def _seed():
        ...
        JobStartedReceipt.objects.bulk_create(objs)
...
async def test_pick_miner_for_job__miner_becomes_eligible_after_one_finished_receipt(
    ...
    await sync_to_async(JobStartedReceipt.objects.all().delete)()
    await sync_to_async(JobFinishedReceipt.objects.all().delete)()
    ...
    def _seed():
        ...
        JobStartedReceipt.objects.bulk_create(started_objs)
        # Finish one job to free a slot
        finished_job = started_objs[0]
...
async def test_pick_miner_for_job__collateral_filter_keeps_subset(add_allowance):
    ...
    miner_obj, created = await sync_to_async(MinerModel.objects.get_or_create)(
    ...
    if not created and miner_obj.collateral_wei < 100:  # ensure it meets threshold
        ...
        await sync_to_async(miner_obj.save)()
