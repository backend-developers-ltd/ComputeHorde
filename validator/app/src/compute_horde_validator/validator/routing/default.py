...
def _pick_miner_for_job_v2(request: V2JobRequest) -> JobRoute:
    ...
    def miner_sort_key(miner_tuple: tuple[str, float]) -> tuple[float, float]:
        miner_hotkey, available_allowance = miner_tuple
        return per_executor_scores.get(miner_hotkey, 0), available_allowance

    # Sort suitable miners by per-executor reliability (higher is better), with available allowance as a tie-breaker.
