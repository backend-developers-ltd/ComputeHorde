...
class MinerClient:
    ...
    def _get_retry_delay(self, attempt: int) -> float:
        return self.base_retry_delay * float(2**attempt) + random.uniform(0, self.retry_jitter)
    ...
    def connect(self) -> None:
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt > 0:
                    logger.info(
                        "Connected to miner %s after %d retries", self.miner_hotkey, attempt
                    )
                return
                ...
                logger.info(
                    "Retrying connection to miner %s in %.2fs (attempt %d/%d)",
                    self.miner_hotkey,
                    delay,
                    attempt + 1,
                    self.max_retries,
    ...
    def send(self, msg: ValidatorToMinerMessage) -> None:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                delay = self._get_retry_delay(attempt)
                logger.info(
                    "Could not send msg to miner %s, reconnecting in %.2fs (attempt %d/%d)",
                    self.miner_hotkey,
                    delay,
                    attempt + 1,
                    self.max_retries,
    ...
    def recv(self, timeout: float) -> MinerToValidatorMessage:
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                raw = self.ws.recv(timeout=timer.time_left(), decode=False)
                ...
                # the caller handles recv timeouts
                raise
            except (websockets.WebSocketException, OSError):
                if attempt >= self.max_retries - 1:
                    raise
                delay = self._get_retry_delay(attempt)
                logger.info(
                    "Could not receive msg from miner %s, reconnecting in %.2fs (attempt %d/%d)",
                    self.miner_hotkey,
                    delay,
                    attempt + 1,
                    self.max_retries,
...
class SyncOrganicJobDriver:
    ...
    def _handle_miner_horde_failed_message(self, msg: V0HordeFailedRequest) -> DriverState:
    ...
    def _wait_for_message(
        ...
        """
        while True:
            time_left = min(deadline.time_left(), self._global_deadline - time.time())
            if time_left <= 0:
            ...
            try:
                msg = self.miner_client.recv(time_left)
            except TimeoutError:
            ...
            if isinstance(msg, V0HordeFailedRequest):
                return self._handle_miner_horde_failed_message(msg)
    ...
    def _wait_job_accepted(self) -> DriverState:
        ...
        result = self._wait_for_message(
            V0AcceptJobRequest,
            deadline=Timer(self.reservation_time_limit),
    ...
    def _wait_executor_ready(self) -> DriverState:
        ...
        result = self._wait_for_message(
            V0ExecutorReadyRequest,
            deadline=Timer(readiness_time_limit),
