...
class Receipts(ReceiptsBase):
    ...
    async def _fetch_miners(self, hotkeys: list[str]) -> list[tuple[str, str, int]]:
        ...
        return [
            item
            async for item in Miner.objects.filter(hotkey__in=hotkeys).values_list(
                "hotkey", "address", "port"
            )
        ]
    ...
    async def _fetch_receipts_for_range(self, start_block: int, end_block: int) -> list[Receipt]:
        ...
        return [
            receipt_data.to_receipt()
            async for receipt_data in JobFinishedReceipt.objects.filter(
