...
class MockAsyncSubtensor:
    def __init__(self, raise_on_metagraph=False, num_neurons=10, block_num=500):
        self.num_neurons = num_neurons
        self.block_num = block_num
        self.raise_on_metagraph = raise_on_metagraph

    async def initialize(self):
        return self

    async def metagraph(self, netuid, lite):
        if self.raise_on_metagraph:
            raise Exception("Simulated metagraph failure")
        return MockMetagraph(num_neurons=self.num_neurons, block_num=self.block_num)
